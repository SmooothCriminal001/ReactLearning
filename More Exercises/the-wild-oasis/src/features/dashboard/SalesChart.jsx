...
export default function SalesChart({ bookings, numDays }) {
  ...
  const dateFormatString = "MMM dd";
  const dateFormatStringWithYear = "MMM dd yyyy";
  ...
  const data = intervalDays.map((eachDate) => {
    return {
      label: format(eachDate, dateFormatString),
      totalSales: bookings
        .filter((eachBooking) =>
          isSameDay(eachDate, new Date(eachBooking.created_at))
        )
  ...
  return (
    <StyledSalesChart>
      <Heading as="h2">
        Sales from {formatDate(intervalDays.at(0), dateFormatStringWithYear)}{" "}
        &mdash;
        {formatDate(intervalDays.at(-1), dateFormatStringWithYear)}
      </Heading>
